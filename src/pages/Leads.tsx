...
//TYPES
import { InputProps, LeadsPostData, LeadsData, MessageProps } from '@/types'
...
function Leads() {
  ...
  useEffect(() => {
    if (createLeadsData?.id) {
      ...
      setCreateMessage({
        msg: 'Não foi possível realiazar a operação!',
        type: 'error',
      })
    } else {
      clearMessage()
      ...
      <Container className="mb-2" maxWidth="lg">
        <Grid container spacing={4}>
          <Grid item xs={12} sm={7}>
            <CardComponent
              className={leadsLoading ? 'skeleton-loading skeleton-loading-mh2' : ''}
            >
              {!leadsError && !leadsLoading && (
                <>
                  ...
                  {leadsData?.length ? (
                    ...
                    <StyledSpan>Nenhum lead cadastrado!</StyledSpan>
                  )}
                </>
              )}
            </CardComponent>
